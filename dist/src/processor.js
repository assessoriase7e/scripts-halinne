...
import { PATH_BRANCO, PATH_MODELO, PATH_OUT, PATH_NOT_FOUND, RECURSIVE_SEARCH, COPY_FILES, MOTHER_FOLDER, MIN_SIMILARITY, MAX_CONCURRENT_REQUESTS, } from "./config.js";
import { cosineSimilarity } from "./imageProcessor.js";
import { verifySimilarity } from "./openaiAPI.js";
import { moveFile, listImageFiles, generateDestinationName, getImageEmbedding, } from "./utils.js";
/**
 * Processa imagens de uma pasta e gera embeddings
 */
export async function processImages(folderPath, cache, folderName) {
    console.log(`═══════════════════════════════════════════`);
    console.log(`📸 PROCESSANDO IMAGENS ${folderName.toUpperCase()} (PARALELO COM CACHE)`);
    console.log(`═══════════════════════════════════════════\n`);
    const embeddings = {};
    ...
    console.log(`Processamento paralelo com até ${MAX_CONCURRENT_REQUESTS} requisições simultâneas\n`);
    // Processar imagens em paralelo com controle de concorrência
    const processImagePromises = files.map(async (imageInfo, index) => {
        console.log(`[INÍCIO] 📍 ${imageInfo.fileName} (${index + 1}/${files.length})`);
        if (imageInfo.relativePath !== imageInfo.fileName) {
            console.log(`    📂 Subpasta: ${path.dirname(imageInfo.relativePath)}`);
        }
        try {
            const { embedding, analysis } = await getImageEmbedding(imageInfo.filePath, cache);
 ...
 */
export async function compareAndGroup(embBranco, embModelo) {
    console.log("\n═══════════════════════════════════════════");
    console.log("🔍 COMPARANDO E AGRUPANDO IMAGENS (PARALELO)");
    console.log("═══════════════════════════════════════════\n");
    const matchedModelo = new Set(); // Rastrear imagens de modelo que foram pareadas
    const matchedModeloLock = new Set(); // Lock para evitar concorrência na movimentação
    let notFoundCount = 0;
    let successfulMatches = 0;
    // Função para processar uma imagem branca com controle de concorrência
    const processImageComparison = async (fnameWhite, dataW, index, total) => {
        console.log(`[${index + 1}/${total}] ✨ Iniciando comparação: ${fnameWhite}`);
        ...
        if (bestScore >= MIN_SIMILARITY && bestMatch) {
            console.log(`   🎯 Candidato encontrado (${(bestScore * 100).toFixed(1)}%): ${bestMatch.filename}`);
            ...
            try {
                ...
                const isActuallySimilar = await verifySimilarity(dataW.imageInfo.filePath, bestMatch.imageInfo.filePath, bestScore);
                if (isActuallySimilar) {
                    console.log(`   ✅ Match confirmado!`);
                    const bar = "█".repeat(Math.round(bestScore * 20));
                    console.log(`   [${bar.padEnd(20, "░")}] ${(bestScore * 100).toFixed(1)}% - ${bestMatch.filename}`);
                    // Marcar como pareada permanentemente
                    matchedModelo.add(bestMatch.filename);
                    // Gerar nome da pasta de destino
                    const destinationName = generateDestinationName(dataW.imageInfo, MOTHER_FOLDER);
                    const folderPath = path.join(PATH_OUT, destinationName);
                    await fs.mkdir(folderPath, { recursive: true });
                    // COPIAR ou MOVER imagem branca
                    await moveFile(dataW.imageInfo.filePath, path.join(folderPath, fnameWhite), COPY_FILES);
                    // COPIAR ou MOVER apenas o melhor match
                    await moveFile(bestMatch.imageInfo.filePath, path.join(folderPath, bestMatch.filename), COPY_FILES);
                    ...
                    await fs.writeFile(path.join(folderPath, "analysis.json"), JSON.stringify(detailedData, null, 2));
                    return { success: true, type: "match", filename: fnameWhite };
                }
                else {
                    console.log(`   ❌ Verificação visual falhou - não são similares o suficiente`);
                    console.log(`   📦 Movendo para not_found/${PATH_BRANCO}/`);
                    // COPIAR ou MOVER para not_found
                    await moveFile(dataW.imageInfo.filePath, path.join(PATH_NOT_FOUND, PATH_BRANCO, fnameWhite), COPY_FILES);
                    ...
                    await fs.writeFile(path.join(PATH_NOT_FOUND, PATH_BRANCO, `${path.parse(fnameWhite).name}.json`), JSON.stringify(notFoundData, null, 2));
                    return {
                        success: false,
            ...
            console.log(`   ❌ Nenhum match válido (melhor: ${(bestScore * 100).toFixed(1)}% < ${(MIN_SIMILARITY * 100).toFixed(0)}%)`);
            console.log(`   📦 Movendo para not_found/${PATH_BRANCO}/`);
            // COPIAR ou MOVER para not_found
            await moveFile(dataW.imageInfo.filePath, path.join(PATH_NOT_FOUND, PATH_BRANCO, fnameWhite), COPY_FILES);
            // Salvar informações sobre por que não foi encontrado match
    ...
    const comparisonPromises = Object.entries(embBranco).map(([fnameWhite, dataW], index) => processImageComparison(fnameWhite, dataW, index, Object.entries(embBranco).length));
    ...
    for (const [fnameMod, dataM] of Object.entries(embModelo)) {
        if (!matchedModelo.has(fnameMod)) {
            ...
            // COPIAR ou MOVER para not_found
            await moveFile(dataM.imageInfo.filePath, path.join(PATH_NOT_FOUND, PATH_MODELO, fnameMod), COPY_FILES);
            ...
            await fs.writeFile(path.join(PATH_NOT_FOUND, PATH_MODELO, `${path.parse(fnameMod).name}.json`), JSON.stringify(unpairedData, null, 2));
            unpairedModelo++;
        }
    }
    return {
 ...
 */
export function displayFinalSummary(successfulMatches, notFoundCount, unpairedModelo) {
    console.log("\n═══════════════════════════════════════════");
    ...
    console.log(`   - Ajuste MIN_SIMILARITY no código se necessário (atual: ${(MIN_SIMILARITY * 100).toFixed(0)}%)\n`);
}
