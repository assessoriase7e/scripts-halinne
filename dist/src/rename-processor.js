...
import { initDatabase, EmbeddingCache } from "./cache.js";
import { getImageEmbedding } from "./utils.js";
/**
 * Atualiza o cache de embeddings com os novos nomes dos arquivos
 */
export async function updateCacheWithNewNames(processedFiles) {
    console.log("\n═══════════════════════════════════════════");
    ...
    try {
        ...
        for (const file of processedFiles) {
            ...
            try {
                ...
                if (!newPath)
                    continue;
                console.log(`🔄 Atualizando: ${path.basename(oldPath)} → ${path.basename(newPath)}`);
                ...
                if (cached) {
                    ...
                    console.log(`   📝 Gerando embedding para: ${path.basename(newPath)}`);
                    try {
                        const { embedding, analysis } = await getImageEmbedding(newPath, cache);
                        console.log(`   ✅ Embedding gerado e cacheado para: ${path.basename(newPath)}`);
                        updatedCount++;
                    }
                    catch (error) {
                        console.log(`   ⚠️ Erro ao gerar embedding: ${error.message}`);
                        errorCount++;
                    }
                }
            }
            catch (error) {
                console.error(`   ❌ Erro ao atualizar cache para ${file.fileName}: ${error.message}`);
                errorCount++;
            }
        }
 ...
 */
export async function checkCacheStatus(filePath) {
    try {
        const db = await initDatabase();
        const cache = new EmbeddingCache(db);
 ...
 */
export async function generateEmbeddingsForDirectory(dirPath, recursive = true) {
    console.log("\n═══════════════════════════════════════════");
    ...
    try {
        ...
        for (let i = 0; i < imageFiles.length; i++) {
            ...
            console.log(`[${i + 1}/${imageFiles.length}] 📸 ${path.basename(imagePath)}`);
            try {
                ...
                if (cached) {
                    ...
                    // Gerar embedding
                    const { embedding, analysis } = await getImageEmbedding(imagePath, cache);
...
//# sourceMappingURL=rename-processor.js.map