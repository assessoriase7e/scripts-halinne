 ...
 */
export function identifyImageType(fileName, filePath) {
    const lowerFileName = fileName.toLowerCase();
    const lowerFilePath = filePath.toLowerCase();
    // Verificar se é produto na pedra
    if (PATTERNS.PRODUCT_ON_STONE.test(lowerFileName) ||
        PATTERNS.PRODUCT_ON_STONE.test(lowerFilePath)) {
        return "PRODUCT_ON_STONE";
    }
    // Verificar se é cópia/adicional
    if (PATTERNS.ADDITIONAL_PHOTO.test(lowerFileName) ||
 ...
 */
export function generateNewFileName(fileName, filePath, code, imageType, counters) {
 ...
 */
export async function listImageFiles(dirPath, recursive = true) {
    try {
        const imageFiles = [];
        ...
        log("error", `Erro ao ler pasta ${dirPath}: ${error.message}`);
        return [];
    }
}
/**
...
export async function moveFile(src, dest, copy = COPY_FILES) {
    try {
        ...
        if (DRY_RUN) {
            log("info", `[DRY RUN] ${copy ? "Copiaria" : "Moveria"}: ${path.basename(src)} → ${dest}`);
            return;
        }
        if (copy) {
        ...
        log("error", `Erro ao ${copy ? "copiar" : "mover"} arquivo: ${error.message}`);
        throw error;
    }
}
/**
 ...
 */
export function createReport(processedFiles, errors) {
    const report = {
 ...
 */
export async function saveReport(report, outputPath = "rename-report.json") {
    try {
        await fs.writeFile(outputPath, JSON.stringify(report, null, 2));
        log("info", `Relatório salvo em: ${outputPath}`);
    }
    catch (error) {
        log("error", `Erro ao salvar relatório: ${error.message}`);
    }
}
//# sourceMappingURL=rename-utils.js.map