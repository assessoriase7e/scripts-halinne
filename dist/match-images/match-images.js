import { PATH_BRANCO, PATH_MODELO, MIN_SIMILARITY, TOP_N, MAX_IMAGE_SIZE, MAX_CONCURRENT_REQUESTS, REQUEST_DELAY, CACHE_DB, COPY_FILES, RECURSIVE_SEARCH, KEEP_ORIGINAL_NAMES, MOTHER_FOLDER, } from "../src/config.js";
import { initDatabase, EmbeddingCache } from "../src/cache.js";
import { processImages, compareAndGroup, prepareOutputFolders, displayFinalSummary, } from "../src/processor.js";
/**
 * Função principal
 */
async function main() {
    ...
    let cache = null;
    try {
        console.log("🚀 Iniciando processamento de joias (PARALELO COM CACHE)...\n");
        console.log(`⚙️  Configurações:`);
        console.log(`   - Similaridade mínima: ${(MIN_SIMILARITY * 100).toFixed(0)}%`);
        ...
        console.log(`   - Manter nomes originais: ${KEEP_ORIGINAL_NAMES ? "Sim" : "Não"}`);
        ...
        const embBranco = await processImages(PATH_BRANCO, cache, "em fundo branco");
        ...
        displayFinalSummary(results.successfulMatches, results.notFoundCount, results.unpairedModelo);
    }
    catch (error) {
        console.error("\n═══════════════════════════════════════════");
        ...
        if (error.code === "ENOENT") {
            console.error("💡 Verifique se as pastas de entrada existem e contêm imagens.");
        }
        process.exit(1);
    }
    finally {
...
// Executar o script
main();
//# sourceMappingURL=match-images.js.map