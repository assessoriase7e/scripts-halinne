...
import { INPUT_DIR, OUTPUT_DIR, RECURSIVE_SEARCH, COUNTERS, COPY_FILES, DRY_RUN, } from "../src/rename-config.js";
import { log, extractCode, identifyImageType, generateNewFileName, listImageFiles, moveFile, createReport, saveReport, } from "../src/rename-utils.js";
...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < imageFiles.length; i++) {
            ...
            log("info", `[${i + 1}/${imageFiles.length}] Processando: ${imageInfo.fileName}`);
            try {
                ...
                try {
                    ...
                    const aiAnalysis = await analyzeImageType(imageInfo.filePath, imageInfo.fileName);
                    imageType =
                        aiAnalysis.type === "VARIANT" ? aiAnalysis : aiAnalysis.type;
                    log("debug", `   Tipo identificado pela IA: ${JSON.stringify(imageType)} (confiança: ${(aiAnalysis.confidence * 100).toFixed(0)}%)`);
                    log("debug", `   Justificativa: ${aiAnalysis.reasoning}`);
                }
                catch (error) {
                    log("warn", `   ⚠️ Erro na análise IA, usando fallback: ${error.message}`);
                    imageType = identifyImageType(imageInfo.fileName, imageInfo.filePath);
                    log("debug", `   Tipo identificado (fallback): ${JSON.stringify(imageType)}`);
                }
                // Gerar novo nome do arquivo
                const newFileName = generateNewFileName(imageInfo.fileName, imageInfo.filePath, code, imageType, COUNTERS);
                ...
                const errorMsg = `Erro ao processar ${imageInfo.fileName}: ${error.message}`;
                log("error", errorMsg);
        ...
        console.log(`   📁 Pastas criadas: ${new Set(processedFiles.filter((f) => f.success).map((f) => f.code)).size}`);
        console.log(`\n📂 Diretórios:`);
        ...
        if (errors.length > 0) {
            ...
            if (errors.length > 5) {
                console.log(`   ... e mais ${errors.length - 5} erros (veja o relatório completo)`);
            }
        }
    }
    catch (error) {
        ...
        if (error.code === "ENOENT") {
            console.error("💡 Verifique se o diretório de entrada existe e contém imagens.");
        }
        process.exit(1);
    }
}
// Executar o script
main();
//# sourceMappingURL=rename-images.js.map