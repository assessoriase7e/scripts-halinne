...
/**
 * Verifica se um arquivo já está em uma subpasta com código
 */
function isInCodeFolder(filePath: string, categoryDir: string): boolean {
  const relativePath = path.relative(categoryDir, filePath);
  const parts = relativePath.split(path.sep);

  // Se tem mais de uma parte no caminho, está em subpasta
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const entries = await fs.readdir(ORGANIZED_DIR, { withFileTypes: true });

    for (const entry of entries) {
      if (entry.isDirectory() && /^\[.+\]\[.+\]$/.test(entry.name)) {
        categoryFolders.push(path.join(ORGANIZED_DIR, entry.name));
      }
    }
    ...
    const looseFiles = await findLooseFiles(ORGANIZED_DIR);
